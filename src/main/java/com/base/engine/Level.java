...
public class Level
{
    ...
    public Level(String fileName, String textureName)
    {
        //this.player = player;
        level = new Bitmap(fileName).flipY();
    ...
    private float[] calcTexCoords(int value)
    {
        ...
        result[0] = 1 - (float)texX /(float)NUM_TEX_EXP;
        result[1] = result[0] - 1/(float)NUM_TEX_EXP;
        result[3] = 1 - (float)texY /(float)NUM_TEX_EXP;
        result[2] = result[3] - 1/(float)NUM_TEX_EXP;

        return result;
    }

    private void addVertices(ArrayList<Vertex> vertices, int i, int j, float offset, boolean y, boolean z, boolean x, float[] texCoords)
    {
        if(x && z)
        {
    ...
    private void addSpecial(int blueValue, int x, int y)
    {
        ...
        if(blueValue == 192)
            medkits.add(new Medkit(new Vector3f((x + 0.5f) * SPOT_WIDTH, 0.0f, (y + 0.5f) * SPOT_LENGTH)));

        if(blueValue == 97)
            exitPoints.add(new Vector3f((x + 0.5f) * SPOT_WIDTH, 0.0f, (y + 0.5f) * SPOT_LENGTH));
    }
